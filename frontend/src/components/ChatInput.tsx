            ...
            if (items[i].type.startsWith('image/')) {
                ...
                if (file) {
                    ...
                    try {
                        ...
                        console.error('Clipboard image upload error:', err);
                    } finally {
                        setIsUploading(false);
                    }
                }
    ...
    return (
        <div className="input-area">
            ...
            <input
                ...
                accept=".pdf,.docx,.doc,.txt,.md,.csv,.py,.json,.js,.ts,.html,.css,.png,.jpg,.jpeg,.webp,.gif,.bmp"
                multiple
                style={{ display: 'none' }}
            />
                            ...
                            {attachments.map((att, idx) => (
                                <div key={idx} className="attachment-chip">
                                    {att.file_type === 'image' && att.image_url ? (
                                        <img src={att.image_url} alt={att.filename} className="attachment-chip__thumb" />
                                    ) : (
                        ...
                        placeholder={
                            isListening
                                ...
                                : attachments.length > 0
                                    ...
                                    : "Message Aibou (or paste images / speak with Mic)..."
                        }
                        rows={1}
